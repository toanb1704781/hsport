...
@section('title', 'Quản lý các thương hiệu')
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        ...
        <div class="col-sm-12">
          <h2 class="main-title">DANH SÁCH CÁC tHƯƠNG HIỆU GIÀY</h2>
        </div>
        <div class="col-sm-12">
          <ol class="breadcrumb float-sm-right">
            <a class="btn btn-success btn-sm" href="{{route('thuong-hieu.create')}}">
          ...
          <tbody>
            @foreach ($brand as $brand)
              <tr>
                ...
                <td>
                  <img src="{{asset($brand->brandAvatar)}}" alt="Brand Avatar" style="width:150px; height:130px">
                </td>
                <td>
                  <form action="{{route('thuong-hieu.destroy', ['thuong_hieu' => $brand->brandID])}}" method="post">
                    @csrf
                    @method('delete')
                    <a class="btn btn-primary btn-sm" href="{{route('thuong-hieu.edit',['thuong_hieu' =>  $brand->brandID])}}">
                      <i class="far fa-edit"></i>
                    </a>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#brand{{$brand->brandID}}">
                      <i class="fas fa-trash"></i>
                    </button>
                    <div class="modal fade" id="brand{{$brand->brandID}}">
                      <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h4 class="modal-title">
                              Bạn có muốn xóa <strong>{{$brand->brandName}}</strong> không?
                            </h4>
                          </div>
