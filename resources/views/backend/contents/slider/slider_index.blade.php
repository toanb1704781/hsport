...
@section('content')
  ...
  <div class="card">
    <section class="content-header">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-12">
            <ol class="breadcrumb float-sm-right">
              <a class="btn btn-success btn-sm" href="{{route('slider.create')}}">
      ...
      <div class="col-sm-12">
        <h2 class="main-title">DANH SÁCH SLIDER HIỂN THỊ</h2>
      </div>
    </section>
          ...
          <tbody>
            @foreach ($slider as $slider)
              <tr>
                ...
                <td>
                  <img src="{{asset($slider->sliderIMG)}}" alt="" style="height:100px; weight:250px">
                </td>
                <td>
                  {{$slider->sliderDesc}}
                </td>
                ...
                <td>
                  {{$slider->employee->information->name ?? $slider->employee->username}} <br>
                  @if ($slider->employee->roleID == 1)
                    <span class="badge badge-danger">Giám đốc</span>      
                  @else
                    @if ($slider->employee->roleID == 2)
                ...
                <td>
                  <form action="{{route('slider.destroy', ['slider' => $slider->sliderID])}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('delete')
                    <a class="btn btn-primary btn-sm" href="{{route('slider.edit', ['slider' => $slider->sliderID])}}">
                      <i class="far fa-edit"></i>
                    </a>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#slider{{$slider->sliderID}}">
                      <i class="fas fa-trash"></i>
                    </button>
                    <div class="modal fade" id="slider{{$slider->sliderID}}">
                      <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h4 class="modal-title">
                              Bạn có muốn xóa slider <strong>{{$slider->sliderDesc}}</strong> không?
                            </h4>
                          </div>
