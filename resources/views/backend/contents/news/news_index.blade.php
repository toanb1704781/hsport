    ...
    <section class="content-header">
      ...
      <div class="col-sm-12">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">Danh sách các tin tức</li>
        </ol>
        <hr>
      </div>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <a class="btn btn-success btn-sm" href="{{route('tin-tuc.create')}}">
          ...
          <tbody>
            @foreach ($allnews as $news)
              <tr>
                ...
                <td>
                    <img src="{{asset($news->newsIMG)}}" alt="" style="height:100px; weight:100px">
                </td>
                <td>
                    {{$news->newsTitle}}
                </td>
                {{--  <td>
                  {{$news->employee->information->name ?? $news->employee->name}}  <br>
                  @if ($news->employee->roleID == 1)
                    <span class="badge badge-danger">Giám đốc</span>      
                  @else
                    @if ($news->employee->roleID == 2)
                ...
                <td>
                  <form action="{{route('tin-tuc.destroy', ['tin_tuc' => $news->newsID])}}" method="post">
                    @csrf
                    @method('delete')
                    <a class="btn btn-primary btn-sm" href="{{route('tin-tuc.edit',['tin_tuc' => $news->newsID])}}">
                      <i class="far fa-edit"></i>
                    </a>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#news{{$news->newsID}}">
                      <i class="fas fa-trash"></i>
                    </button>
                    <div class="modal fade" id="news{{$news->newsID}}">
                      <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h4 class="modal-title">
                              Bạn có muốn xóa <strong>{{$news->newsTitle}}</strong> không?
                            </h4>
                          </div>
