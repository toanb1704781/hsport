...
@section('content')
    <section class="content-header">
      <div class="col-sm-12">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">Cập nhật khuyến mãi</li>
        </ol>
        <hr>
      </div>
    </section>
    ...
    <section class="content">
        <div class="card card-primary">
            ...
            <form action="{{route('khuyen-mai.update', ['khuyen_mai' => $discount->disID])}}" method="post">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="form-group">
                        <label for="disID">Mã khuyến mãi</label>
                        <input type="text" class="form-control" id="disID" name="disID" value="{{$discount->disID}}">
                    </div>
                    <div class="form-group">
                      <label for="disDesc">Mô tả khuyến mãi</label>
                      <input type="text" class="form-control" id="disDesc" name="disDesc" value="{{$discount->disDesc}}">
                  </div>
                  <div class="form-group">
                    <label for="disCondition">Áp dụng đơn (VNĐ)</label>
                    <input type="number" class="form-control" id="disCondition" name="disCondition" value="{{$discount->disCondition}}" min="0" step="10000">
                  </div>
                    <div class="form-group">
                        <label for="disQuan">Số lượng khuyến mãi</label>
                        <input type="number" class="form-control" id="disQuan" name="disQuan" value="{{$discount->disQuan}}" min="0">
                    </div>
                    <div class="form-group">
                        <label for="discount">Giảm giá (VNĐ)</label>
                        <input type="number" class="form-control" id="discount" name="discount" value="{{$discount->discount}}" min="0">
                    </div>
                    <div class="form-group">
                        <label for="startDate">Ngày bắt đầu</label>
                        <input type="date" class="form-control" id="startDate" name="startDate" value="{{$discount->startDate}}">
                    </div>
                    <div class="form-group">
                        <label for="endDate">Ngày kết thúc</label>
                        <input type="date" class="form-control" id="endDate" name="endDate" value="{{$discount->endDate}}" placeholder="Nhập ngày kết thúc">
                    </div>
                </div>
